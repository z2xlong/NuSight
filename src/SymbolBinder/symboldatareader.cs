...
namespace SymbolBinder
{
    ...
    /// </summary>
    [Serializable]
    public class SymbolDataReader : Disposable
    {
        string _assemblyPath;

        SymbolFormat _symFormat;

        // Expand known attributes when possible
        bool _expandAttributes;

        // Maps files to ids. 
        Dictionary<string, int> m_fileMapping;

        Assembly _assembly;
        static HashSet<string> _loadedAssembly = new HashSet<string>();
        ...
        private class CDIReader : BinaryReader
        {
            ...
            private CDIReader(byte[] val)
                : base(new MemoryStream(val), Encoding.Unicode)
            {
            }
